...
interface FormData { name: string; email: string; useCase: string; teamSize: string; }
...
const CHECK = (
  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--color-secondary-dark)" strokeWidth="2.5" aria-hidden="true">
...
function Perk({ text }: { text: string }) {
  return (
    <div className="flex items-start gap-2.5">
      ...
      <p className="font-body text-sm leading-relaxed text-(--color-text-muted)">{text}</p>
    </div>
  );
}
    ...
    >
      <div className="mx-auto max-w-300">
        ...
        >
          ...
          <h2 id="cta-heading" className="font-heading text-4xl font-medium text-(--color-text) md:text-5xl">
            Ship your first{' '}
          ...
          <p className="mx-auto mt-3 max-w-135 font-body text-base leading-[1.75] text-(--color-text-muted)">
            Free forever for individuals. No credit card. No setup fees. Just open-source software that works.
          </p>
        </motion.div>

        <div className="grid gap-8" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 380px), 1fr))' }}>
          {/* Left — perks */}
          <motion.div
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              {[{ val: '48k+', sub: 'GitHub Stars' }, { val: '3.2M', sub: 'Monthly Downloads' }, { val: '180+', sub: 'Contributors' }].map((s) => (
                <div key={s.sub} className="neu-card-sm min-w-22.5 flex-1 py-3 text-center">
                  <p className="font-heading text-[1.35rem] font-semibold leading-none text-(--color-text)">{s.val}</p>
                  <p className="mt-1 font-body text-[0.64rem] uppercase tracking-[0.08em] text-(--color-text-muted)">{s.sub}</p>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            {submitted ? (
              ...
              >
                ...
                <h3 className="font-heading text-2xl font-medium text-(--color-text)">You're on the list!</h3>
                <p className="max-w-85 font-body text-sm leading-relaxed text-(--color-text-muted)">
                  Welcome aboard, <strong>{form.name}</strong>. We've sent a confirmation to <strong>{form.email}</strong> with next steps.
                </p>
                <div className="mt-2 flex flex-wrap justify-center gap-3">
                  <a href="https://github.com/sailorlabs-in" target="_blank" rel="noopener noreferrer" className="btn-cta text-xs! px-4! py-2.5!">
                    Browse GitHub
                  </a>
                  <button onClick={() => { setForm(INITIAL); setSubmitted(false); }} className="btn-outline text-xs! px-4! py-2.5! cursor-pointer">
                    Register another
                  </button>
                </div>
              </motion.div>
            ) : (
              <form onSubmit={onSubmit} noValidate aria-label="Get started registration form">
                <h3 className="mb-6 font-heading text-[1.45rem] font-medium text-(--color-text)">
                  Create your free account
                </h3>
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="cta-name" className="font-body text-[0.8rem] font-semibold text-(--color-text)">
                      Full name <span aria-hidden="true" className="text-(--color-primary)">*</span>
                    </label>
                    <input id="cta-name" name="name" type="text" required autoComplete="name"
                      placeholder="Jane Developer" value={form.name} onChange={onChange}
                      className="form-input" aria-required="true" />
                  </div>

                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="cta-email" className="font-body text-[0.8rem] font-semibold text-(--color-text)">
                      Work email <span aria-hidden="true" className="text-(--color-primary)">*</span>
                    </label>
                    <input id="cta-email" name="email" type="email" required autoComplete="email"
                      placeholder="jane@company.com" value={form.email} onChange={onChange}
                      className="form-input" aria-required="true" />
                  </div>

                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="cta-usecase" className="font-body text-[0.8rem] font-semibold text-(--color-text)">
                      Primary use case
                    </label>
                    <select id="cta-usecase" name="useCase" value={form.useCase} onChange={onChange} className="form-input">
                      {USE_CASES.map((u) => <option key={u.value} value={u.value} disabled={!u.value}>{u.label}</option>)}
                    </select>
                  </div>

                  <div className="flex flex-col gap-1.5">
                    <label htmlFor="cta-team" className="font-body text-[0.8rem] font-semibold text-(--color-text)">
                      Team size
                    </label>
                    <select id="cta-team" name="teamSize" value={form.teamSize} onChange={onChange} className="form-input">
                      {TEAM_SIZES.map((t) => <option key={t.value} value={t.value} disabled={!t.value}>{t.label}</option>)}
                    </select>
                  </div>
                </div>
